...
They also accept `options.onProgress` for line/variant progress updates. The
public `synchronize()` wrapper forwards `parameters.signal` and
`parameters.onProgress` for reference-template engines; see
