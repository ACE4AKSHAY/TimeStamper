...
for (const lyric of lyricFiles) {
  ...
  const monotonic = timestamped.length > 0 ? timestamped.every((line, index) => index === 0 || line.startTime >= timestamped[index - 1].startTime) : null;
...
const summary = { audioFiles: audioFiles.length, lyricFiles: lyricFiles.length, singleCandidate: items.filter((item) => item.pairingStatus === "single_candidate").length, unmatchedLyrics: items.filter((item) => item.pairingStatus === "audio_not_found").length, multipleAudioCandidates: items.filter((item) => item.pairingStatus === "multiple_candidates").length, fullyTimestamped: items.filter((item) => item.timestampStatus === "fully_timestamped").length, monotonicTimestampFiles: items.filter((item) => item.timestampsMonotonic === true).length, probableSpecialVersions: items.filter((item) => item.probableInstrumental).length, readyForManualReview: items.filter((item) => item.readyForManualReview).length };
