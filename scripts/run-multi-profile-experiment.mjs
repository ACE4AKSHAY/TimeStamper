...
const outputPath = process.argv[3] || "benchmarks/results/multi-profile-boundary-dp.json";
...
for (const item of dataset.cases) {
  ...
  const result = synchronize({ lyrics: lines, engine: "multi-profile-boundary-dp", duration: item.duration, parameters: { profiles: item.profiles, weights: dataset.weights, minLength: item.minLength, maxLength: item.maxLength, boundaryWeight: item.boundaryWeight } });
...
const output = { datasetVersion: dataset.datasetVersion, caseCount: dataset.cases.length, method: "multi_profile_boundary_dp", weights: dataset.weights, runtimeMs: performance.now() - started, metrics: scoreTimestamps(predicted, reference), cases, input: inputPath, generatedAt: new Date().toISOString() };
