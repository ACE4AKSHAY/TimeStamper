...
| 123 | Reference-template acquisition | 🟡 | Verified reference timestamps can now produce MFCC line templates and drive target template-DTW alignment; opt-in banded DTW and anchor-free target search now flow end to end; real recordings and confidence calibration remain. |
...
| 144 | Private real-case evaluation harness | 🟡 | Verified case folders can now be batch-decoded and scored across engines without copying audio or lyric text; the manual review checklist now defines the user gate before promoting algorithm results. |
